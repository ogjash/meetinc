...
export const DashboardNavbar = () => {
    ...
    const [ commandOpen, setCommanOpen ] = useState(false)

    useEffect(()=>{
        const down = (e: KeyboardEvent) => {
            if(e.key === "k" && (e.metaKey || e.ctrlKey)){
                e.preventDefault();
                setCommanOpen((open) => !open );
    ...
    return ( 
        <>
            <DashboardCommand open={commandOpen} setOpen={setCommanOpen }/>
            <div className="flex px-4 gap-x-2 items-center py-3 border-b bg-background">
                ...
                <Button 
                    ...
                    variant="outline" 
                    size="sm" 
                    onClick={() => {setCommanOpen((open) => !open)}}
                > 
                    <SearchIcon/>
                    Search
